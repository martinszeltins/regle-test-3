<template>
    <div class="p-5 space-y-5 origin-top-left">
        ...
        <!-- External Errors -->
        <div style="border: 1px solid gray; position: fixed; right: 1070px; top: 20px; max-height: calc(100vh - 100px); width: 300px; font-size: 11px; overflow:auto;padding: 10px;background: #f8f8f8;border-radius: 4px;font-family: monospace;"><pre><b>externalErrors:</b> {{ externalErrors }}</pre></div>
...
<script setup lang="ts">
    ...
    const externalErrors = ref<RegleExternalErrorTree<Form>>({
        ...
        skills: {
            $each: [
                {
                    skillName: ['External error for skills[0].skillName'],
